...
keep = names(trn)[grep('correct|dist$|sdx|wife|mtchs|old|young|namefreq', names(trn))]
trn = trn[, keep, with=F]
...
table(trn$correct, pred_lgt_trn > 0.5)
...
plot(voteshares, sqrt(fill[,2]^2 + fill[,3]^2), type='l')
abline(v=0.5, col='gray', lty=2)
